...
return {
  ...
  -- ⬇️ La partie qui gère l’ampoule
  lightbulb = {
    enable = false,       -- on garde l’ampoule
    sign = true,         -- dans la signcolumn (marge gauche)
    virtual_text = false,-- pas de texte/icone en fin de ligne
    sign_priority = 30,  -- baisse/augmente si ça se bat avec d’autres signs
  },
})
      ...
      vim.keymap.set("n", "<leader>cl", function() vim.lsp.codelens.refresh() end, { desc = "CodeLens: refresh" })
      vim.keymap.set("n", "<leader>cr", function() vim.lsp.codelens.run() end,     { desc = "CodeLens: run under cursor" })
...

  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "gr", function() tb.lsp_references() end,        { desc = "References (dropdown)" })
      vim.keymap.set("n", "gD", function() tb.lsp_definitions() end,       { desc = "Definitions (dropdown)" })
      vim.keymap.set("n", "gI", function() tb.lsp_implementations() end,   { desc = "Implementations (dropdown)" })
    end,
  },
  ...
  -- 5) Lightbulb : ampoule quand une action est dispo
    ...
    config = function()
      ...
      vim.keymap.set({ "n", "v" }, "<A-CR>", require("actions-preview").code_actions,
